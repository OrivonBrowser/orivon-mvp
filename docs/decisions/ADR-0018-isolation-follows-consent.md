...
own reasoning — an ordinary website has no grant and no app storage to protect — is kept intact
...
2. **Wrong axis.** Installation and consent are independent. An app can be loaded this session
   ...
   registers a manifest for it this run — A158's hydration seam exists precisely to make that
   second case true.
...
- **First visit is still unisolated, and that is correct.** Before any grant exists, a
  ...
  which swaps the view — cheap at that point, because there is nothing stored yet.
...
- **Collapse the two registries — nothing may be served without being registered with the
