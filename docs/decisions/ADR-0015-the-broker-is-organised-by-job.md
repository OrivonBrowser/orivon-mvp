...
> directory count: [`ADR-0023`](ADR-0023-src-main-is-organised-by-job.md) groups it into nine
...
file placed on the wrong side becomes *hidden* from audit rather than merely misfiled, which is a worse
...
**Promote `policy/` to `src/policy/`.** Defensible on the facts, since `src/loader/` already imports
...
intent rather than of every file. The owner was offered the alternative, grouping it by what it
