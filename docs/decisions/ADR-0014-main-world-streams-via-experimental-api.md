...
data types are copied and frozen."* A `ReadableStream` is not a function, so it is copied as plain
...
4. The object produced in the main world is a **real, normally-behaving main-world
   `ReadableStream`**, with `instanceof ReadableStream` true against the main world's own global,
