...
> *"fetch → show the capability grant prompt → cache → run from cache thereafter"*, with the grant
...
> permission gauntlet, and deferring to first capability use keeps every prompt tied to something
...
| | installed size |
|---|---|
| Electron runtime (Chromium + Node) | ~150-200 MB, an unavoidable floor |
...
**Open technical risk, to be resolved by spike in week 1 before building on it:** can `webtorrent`
