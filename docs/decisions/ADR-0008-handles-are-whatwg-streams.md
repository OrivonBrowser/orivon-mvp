...
growth under a fast swarm is to hand-roll flow control on an abstraction that was not
...
`Duplex`/`EventEmitter` shapes on top, rather than a thin pass-through. That is accepted
