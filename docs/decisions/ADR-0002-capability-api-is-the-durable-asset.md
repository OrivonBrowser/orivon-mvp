...
- **Run capability-bearing WASM inside the renderer's own V8.** Impossible, recorded because
  ...
  page sandbox, because that sandbox is exactly what is being escaped. This is contradiction B5 in
...
- `fs` is confined to the app's own sandbox directory, with no path escape. Access outside it
  exists only as `fs.userSelected`, routed through an OS file picker, where the user choosing the
...
## Reversibility
- **Cost to reverse:** the *implementation* is cheap to swap, which is the point of this ADR.
