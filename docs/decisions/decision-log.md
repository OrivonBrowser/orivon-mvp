...
| `d-0122` | 2026-09-26 | A gateway a resolver appears to be lying about (A251) is reached directly at a DNS-over-HTTPS-confirmed address, never as a first choice: only after `net.fetch` has failed it with a transport error, and only when no proxy is configured for it. Chosen over turning on secure DNS for the whole browser (would also fix ISP-blocked non-gateway hosts, but sends every hostname the person browses to a third-party resolver instead of just the gateways already trusted) and over doing nothing beyond the reliability fixes below (leaves a line that blocks the default gateways with no `.eth` page at all). Owner | A251, [`ADR-0030`](ADR-0030-a-eth-name-is-an-origin-served-by-a-verifier.md) |
| `d-0123` | 2026-09-26 | Gateway concurrency is per gateway (4), not one pool shared by all of them (was 8); a block fetch hedges a second gateway after 2 s; a proven name keeps serving stale for up to 10 minutes while a single re-prove runs in the background; the light client tolerates one failed refresh within a 2-minute grace window. All four numbers are AI-recommended and provisional -- none calibrated against a real burst beyond the one line this was measured on. AI, provisional | [`ADR-0030`](ADR-0030-a-eth-name-is-an-origin-served-by-a-verifier.md), `../../src/ipfs/limits.ts`, `../../src/verifier-host/serve/sites.ts`, `../../src/verifier-host/light-client/light-client.ts` |
