...
| `d-0052` | 2026-09-22 | The tab that reported a first install which newly registered the origin is reloaded once, after consent, so it runs as the app. AI; the owner declined this option in A146, so it awaits confirmation | A146, A233, `../../src/main/install/README.md` |
...
| `d-0063` | 2026-09-22 | A connect pattern whose host is exactly `localhost` authorises `127.0.0.1` and `::1` at its port; `localhost` is never resolved. AI; a T12 exception awaiting owner confirmation | A214, `../../src/broker/policy/README.md` |
...
| `d-0075` | 2026-09-22 | The shim's `tls` runs over `connectSecure`; options that change who is trusted refuse by name; `rejectUnauthorized: false` is accepted with verification left on. AI; awaits owner confirmation | A226, `../../src/shim/README.md` |
