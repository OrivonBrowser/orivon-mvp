...
| `d-0050` | 2026-09-22 | A dev-granted origin's documents are served the same CSP an installed app's are, appended to the dev server's own. Owner | A211, `../../src/main/install/README.md` |
...
| `d-0060` | 2026-09-22 | `MAX_MANIFEST_BYTES` is 278,400 bytes, so a manifest declaring the most assets a bundle may hold still fits. AI | `../../src/loader/manifest/manifest.ts`, A120 |
...
| `d-0067` | 2026-09-22 | The fs quota counts disk usage: reconciled from disk once per session, `writeFile` charges growth, `rm`/overwrite/`rename` release what they free. AI | A216, `../../src/broker/capabilities/fs.ts` |
...
| `d-0069` | 2026-09-22 | A socket closed cleanly in both directions frees its handle and its allowance slot. AI | `../../src/broker/transport/relay/port-pump.ts` |
...
| `d-0111` | 2026-09-24 | The light client starts at launch; its state shows in a Settings section and in the site-info popover on `.eth` pages. Owner | [`ADR-0031`](ADR-0031-helios-is-the-light-client.md), A253 |
| `d-0112` | 2026-09-24 | ENS resolution and IPFS data gathering are built in, as default providers behind internal interfaces in the canonical shape; opening them to third-party Apps is a later `src/contracts/` change. Owner | [`ADR-0030`](ADR-0030-a-eth-name-is-an-origin-served-by-a-verifier.md), `../../src/resolution/` |
| `d-0113` | 2026-09-24 | Third-party code does not lower Website Level 2: Level 2 asks whether the site's own files are what it published; external code costs judged score and shows as pin coverage. Derived from `d-0108` and the 2026-09-15 amendment of ADR-0006. AI | [`ADR-0006`](ADR-0006-trust-indicator-from-observed-behaviour.md) |
| `d-0114` | 2026-09-25 | A site that meets DDOC is Website Level 2, as the canonical page says, and a hash tree published on the site's own host counts. Owner | [`ADR-0029`](ADR-0029-sites-publish-their-bundle-hash-tree.md), [`ADR-0006`](ADR-0006-trust-indicator-from-observed-behaviour.md), A254 |
| `d-0115` | 2026-09-25 | A `.eth` name reached through a DNSLink is Level 2 too: IPFS content meets DDOC by design, and `d-0114` makes the anchor's strength evidence rather than a condition. Its DNS hop stays unproven in the evidence and leaves D4 unmet. Replaces the plan's derived "a DNSLink name is Level 1". AI, *provisional* until the owner confirms | [`ADR-0030`](ADR-0030-a-eth-name-is-an-origin-served-by-a-verifier.md) |
| `d-0116` | 2026-09-25 | A `.eth` name's ENS record anchors DDOC off the host: the contenthash, for content on IPFS; the same record would carry a hash-pinned app's bundle root, which this build does not load from a `.eth` name. Owner | [`ADR-0029`](ADR-0029-sites-publish-their-bundle-hash-tree.md), A255 |
| `d-0117` | 2026-09-25 | The `.eth` verifier's caches are isolated per top-level site, so no page can time a request to learn which names the person opened. Owner | [`ADR-0030`](ADR-0030-a-eth-name-is-an-origin-served-by-a-verifier.md), A256 |
| `d-0118` | 2026-09-25 | A loopback origin (a loopback literal or a `localhost` name) whose page hints a manifest asks for its permissions in every build, `npm start` and a packaged one included, and is granted without being installed, for the session only. No developer mode, and no proof the address was typed. Relaxes A46 and answers A65. `npm start` stays out of developer mode: the owner rejected that route, since it is the real application. Owner | A46, A65, `../../src/main/install/README.md`, `../development/setup.md` |
| `d-0119` | 2026-09-25 | `net.listen`/`udpBind` split into `local`/`network` scopes, nested under `BindScopes` in the manifest, applying to both TCP and UDP. A ported app asking for every interface under a local-only grant binds loopback instead of being refused. Owner | [`ADR-0034`](ADR-0034-listening-is-local-unless-the-app-declares-the-network.md), A225, `../architecture/security-model.md` T12 |
| `d-0120` | 2026-09-25 | `src/preload/`, `src/shim/`, `src/verifier-host/`, `src/loader/` and `src/broker/` are reorganised into job-named folders, on one rule: a moved file drops the words its new folder already says. Owner | [`ADR-0035`](ADR-0035-src-source-directories-are-organised-by-job.md) |
| `d-0121` | 2026-09-25 | The five-directory reorganisation above lands as one repo-wide sweep on a single branch rather than five, the carve-out A24 asks whether a repo-wide sweep should get. Owner | A24, [`ADR-0035`](ADR-0035-src-source-directories-are-organised-by-job.md), `../development/parallel-work.md` |
| `d-0122` | 2026-09-26 | An app qualifies by running in the Node environment, not by being JavaScript: WebAssembly components are accepted, since Node and an app's renderer both run WebAssembly natively, and the compatibility matrix's gaps are taken case by case. Owner | [`ADR-0036`](ADR-0036-an-app-qualifies-by-running-in-the-node-environment.md), `../architecture/app-compatibility.md`, `../planning/compatibility-matrix.md` |
| `d-0123` | 2026-09-26 | Following d-0122: Rule 8 is retitled "No native modules in Orivon's own dependencies", the property `check:natives` enforces, and states that it bounds this repository and not apps; the matrix's Table 4 row 12 narrows from "app logic is not JavaScript, WASM included" to a standalone WASI program, since WebAssembly called from JavaScript runs. AI | `../../CLAUDE.md` Rule 8, `../../CONTRIBUTING.md`, `../planning/compatibility-matrix.md` Table 4 |
| `d-0124` | 2026-09-26 | The Web3 Score shield shows the site's displayed Website level, coloured red/orange/yellow/green, with a "Web2"/"Web3" label at the ends of the scale; a plain grey outline with no level yet. Owner | [`ADR-0006`](ADR-0006-trust-indicator-from-observed-behaviour.md) |
| `d-0125` | 2026-09-26 | The delivery ladder's D1-D4 rungs are replaced by three levels matching the canonical Connection-to-network scale (red/yellow/green); D2 is exactly the old D4's condition, and nothing automatic reaches D3 in this build. Owner | [`ADR-0006`](ADR-0006-trust-indicator-from-observed-behaviour.md) |
| `d-0126` | 2026-09-26 | A site whose displayed Website level is 4 shows every capability grant without a warning, on every consent surface. Owner | [`ADR-0037`](ADR-0037-a-level-4-site-s-grants-are-shown-without-warnings.md) |
| `d-0127` | 2026-09-26 | A developer-only file (`ORIVON_SCORE_LEVELS_FILE`, gated behind `ORIVON_DEV_ORIGINS=1`) can override the displayed Website level and Delivery level per origin, previewing Level 3/4 before a real provider or peer-to-peer fetching exist; never compiled out, since it grants nothing and reads no more of the environment than developer mode already exposes. Owner | [`ADR-0006`](ADR-0006-trust-indicator-from-observed-behaviour.md), [`ADR-0037`](ADR-0037-a-level-4-site-s-grants-are-shown-without-warnings.md) |
| `d-0128` | 2026-09-26 | The shield drops ADR-0007's cache-provenance signal; that returns with a future "store this Web3site locally" affordance, not built here. Owner | [`ADR-0007`](ADR-0007-cached-bundles-served-at-their-own-origin.md) |
| `d-0129` | 2026-09-26 | Every permission row (the site-info popup, the all-sites panel) carries an icon for the capability, picked-path kind or notification it represents, hand-drawn from lucide path data. Owner | `src/renderer/grant-icons.ts` |
