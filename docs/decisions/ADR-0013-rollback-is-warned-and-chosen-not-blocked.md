...
- **Every later visit where the served version is still below the floor, once the user has chosen
  ...
  It is visible, but it never blocks anything and never requires a click to dismiss or proceed past.
...
`ADR-0009`'s Consequences section), not a UI wording change, and recorded here per `CLAUDE.md` Rule 1
...
failure `ADR-0012` was written to avoid for the unrelated fetch-and-cache step, since training the user
...
Splitting the DECISION this way, `rollback-choice` the first time and `rollback-notice` every time
...
already supplies `grantedPatterns` and `versionFloor`, and this module does not reach into storage
for any of them.
...
- `LoadResult` (`src/loader/index.ts`) gains `LoadNeedsRollbackChoice` (parallel to
  `LoadNeedsReconsent`/`LoadNeedsCapabilityPrompt`), which carries `manifest`/`tree`/`entries` so
  ...
  an acknowledged rollback, and the caller uses this to render the passive notice, never a prompt.
- **Resolved 2026-09-05 (owner decision `d-0017`), was flagged as open below:** choosing "proceed
  ...
  re-prompted." This confirms the recommendation this ADR's implementation already made, since a choice
...
- **What would make us revisit:** evidence that users routinely choose "proceed with the older
  version" without understanding they may be accepting a rollback attack: a real incident, say,
...
behaviour, because a rollback that also widened authority still got the maximally restrictive response.
...
explicit, interactive choice, not a silent install), so there was nothing to escalate it TO. The
