...
| T36 | A web page times a `.eth` request to learn which names the person opened | The verifier's caches are kept per top-level page origin, which the shell stamps on every page's `.eth` request; a request from no page shares nothing, and the host bypasses the HTTP cache. A top-level window opened on a name, and shared concurrency slots, remain as in any browser with partitioned caches (`ADR-0030`) |
