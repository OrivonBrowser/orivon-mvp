...
| T29 | The Web3 Score shield or page claims a Website level the site has not earned | Level 2 only when DDOC holds: every byte of a `.eth` site checked against its CID, or an installed site's files matching its published tree. Level 3 and above only from a provider, or the developer-only override; either is always named as such on the shield's tooltip and the Web3 Score page, never shown as observed. With no provider configured and no override, they show grey `?`. How the anchor is held (a proven name, a DNSLink, the site's own host) is evidence beside the level (`ADR-0006`) |
...
| T39 | A site wrongly shown at Website Level 4 silences its own grants' breadth warnings | `ADR-0037`: no provider exists yet, so the only way to reach Level 4 today is the developer-only override (`src/main/dev/score-levels.ts`), gated behind `ORIVON_DEV_ORIGINS=1` plus a named env file, logged loudly on every use, and never reachable from `window.orivon`, IPC or any other renderer-reachable surface. The words a grant carries never change, only the `⚠` marker and the dialog's icon; a future real provider's Level 4 attestation inherits this same power and must be weighed again when it is wired in |
