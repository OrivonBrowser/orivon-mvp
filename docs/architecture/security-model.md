...
| T8 | Identity key exfiltration | Seed in `safeStorage`, never exposed; apps receive **derived** keys and secrets only (`orivon.id`, and `orivon.secrets` under a distinct salt, [`ADR-0031`](../decisions/ADR-0031-an-app-may-hold-an-origin-bound-secret-in-the-os-keyring.md)); raw export is not a capability at any tier. **Scope of that protection:** `safeStorage` defends against another OS user and against offline disk access, but not against same-user code (T24) |
...
([`ADR-0031`](../decisions/ADR-0031-an-app-may-hold-an-origin-bound-secret-in-the-os-keyring.md))
