...
| Identity seed in the OS keyring; `orivon.secrets`, an app's own origin-bound encrypted secret | `orivon.id` cannot survive a restart without the first half. Owner-requested; needed by two ported apps' own hand-off items (Element's pickle key, AirGap Vault's non-keyring fallback). `ADR-0031` |
