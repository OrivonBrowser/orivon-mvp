...
**`<vision-corpus-repo>`**: Docusaurus site, deployed to
...
- `<vision-corpus-repo> (Copy)`: pre-2026-01-27 snapshot
- `<vision-corpus-repo> (Copy 2)`: 2026-01-27 snapshot
- `<other-doc-clones>`: two further clones

---

## 1b. "OrivonBook": NEWEST statement of the product thesis (work in progress)

**`<book-drafts>`**: a mini-book being
...
**`<early-notes>`**: not version-controlled. Mixed Italian/English.
...
| `technical/orivon-core` | 2026-02-06 | `orivon-core` as a standalone queryable **daemon**, CLI-debuggable. Enumerates module capability sets (Metadata, DNS Resolution, Data Gathering, Account, Crypto, Address List, **Network-core**, Network, Web3 Score provider) and the full permission matrix. Ends with the unanswered heading *"Come Orivon Core si connetterà al browser?"*, meaning how core connects to the browser |
...
**`<prior-mvp>`**: Electron + electron-vite + TypeScript.
...
> **This is a failed MVP and is to be ignored.**
