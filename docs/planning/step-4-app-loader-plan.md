...
**Read first.** [`unattended-build-queue.md`](unattended-build-queue.md) -- this plan finishes
its Phase 4 and adds the three items that Phase 4 assumed someone else had done.
[`../development/unattended-run-protocol.md`](../development/unattended-run-protocol.md) --
...
Owner decision `d-0025` (`ADR-0012`'s 2026-09-13 amendment). After install and before the app's
...
(`A110`)**, so the header is set on the handler's own `Response` -- the handler already fully
controls it. Do not wire `webRequest` and assume it works; that is the exact trap `A110` records.
...
- **The folder picker (`fs.userSelected`)** and **`id.requestIdentity`'s prompt** -- Phase 4
