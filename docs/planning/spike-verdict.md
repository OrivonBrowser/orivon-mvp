...
answer, whether `webtorrent` can run inside a sandboxed Electron renderer over a shimmed
...
while beating the actual product requirement by an order of magnitude, so read past the verdict
field on that one.
...
configured with `dht:false, tracker:false, lsd:false, utp:false`, so no signalling channel of
...
protocol encryption) couldn't run in a renderer, because WebCrypto doesn't usefully provide
...
plaintext fallback), with a piece verified in 480 ms against a Node seeder using native crypto.
...
not privacy against eavesdroppers, and it must never be presented as making torrenting private.
...
non-optional, since its install script falls back to compiling with CMake when no prebuild matches,
