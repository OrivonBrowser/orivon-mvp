...
| A4b Site-level Trustlessity blocked on resolution | **Owner caught this.** DDOC anchors in DNS, forgeable on ICANN domains without DNSSEC → site L2 and DDOC leave the MVP; trustless resolution (D) is a **prerequisite** for them, which reorders the public roadmap. Everything else in the spectrum turned out to be automatic → `ADR-0006`. **Reversed 2026-09-24 (owner, `d-0102`):** trustless resolution is not a prerequisite for DDOC. DDOC is the website axis; whether a DNS answer can be trusted is the connection axis's question, scored separately. DDOC ships as evidence with the site's own host as its anchor → `ADR-0029`. That anchor is *provisional*: a host compromised well enough to rewrite both its files and its tree passes it. **What would settle it:** an off-host record carrying the bundle root (a DNS record as in the vision, or an ENS contenthash), which is out of this build for scope and is C1's question |
...
Now the question of DDOC's *off-host* anchor only: DDOC ships with a same-host anchor
(`ADR-0029`), and A4b's premise that it waited on trustless resolution is reversed. Also unexamined: `Glossario`'s claim that DDOC justifies **self-signed
...
| 3 | **Withdrawn 2026-09-24 (owner, `d-0102`).** This row asked `roadmap.mdx` to make trustless resolution a prerequisite for DDOC and site-level scores. It is not one: DDOC is the website axis, and whether a DNS answer can be trusted belongs to the connection axis (`ADR-0006`'s amendment, `ADR-0029`) | — |
...
**2026-09-24:** a site that publishes its hash tree (`ADR-0029`) offers a cheaper backstop. Its
published root changes with any file, so fetching that one small file and comparing the root with
the pin would notice a release that changes only files. Not built: today the tree is fetched only
after a manifest that is not a 304.
...
Filed 2026-09-24 with `d-0103`. The success metric is 25 h/month of `activeSec`, daily-driver use,
...
Filed 2026-09-24 with `d-0104`. Judged levels ship in build step 7, and the provider need not be
