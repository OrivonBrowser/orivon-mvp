...
|---|---|---|
| The `npm run check:*` gates | **CI, and on demand** | Eleven of them, all run by CI: `check:contracts` (`src/contracts/` complete and referencing only its own siblings — `./errors.js` is fine, `electron`, `node:*` or any package is not), `check:natives` (Rule 8, also on `postinstall`), `check:size` (Rule 2), `check:comments` (Rule 1), `check:secrets`, `check:questions` (no duplicate A-number), `check:vectors`, `check:manifest-parity`, `check:dev-grant-absent`, `check:advisories`, `check:page-globals` (ADR-0021: a global Orivon installs on an app's window must carry the platform's own descriptor, so an app can replace it) |
