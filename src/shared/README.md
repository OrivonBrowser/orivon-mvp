...
**Empty by design.** It exists so that Rule 3 of
[`code-guidelines.md`](../../docs/development/code-guidelines.md) has a legal answer, not
because anything needs to live here yet.
...
never import `src/broker/`, because importing the broker would hand renderer-side code main-process
...
**None.** Every duplicate found in the tree so far sat inside a single directory and belongs
there: `concat`/`encodeField`, for instance, both live in
[`policy/bytes.ts`](../broker/policy/bytes.ts), because both callers are in `src/broker/policy/`.
Nothing in the current tree crosses the `src/broker/` <-> `src/shim/` boundary this directory
exists to serve.
