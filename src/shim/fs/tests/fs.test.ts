// Exercises fs/fs.ts's own wiring -- the module-map target a real
...
// net/tests/net.test.ts/node-dgram.test.ts.
...
function installFakeOrivon (): {
  ...
  ;(globalThis as GlobalWithOrivon).orivon = {
    fs: {
      ...
      // fs/tests/handle.test.ts's job; this stub only proves fs/fs.ts's
...
describe('a relative path is passed to orivon.fs verbatim', () => {
  it('readFile / writeFile / mkdir / stat / rm / rename', async () => {
    ...
    // fs/tests/root.test.ts; this test's own job is proving an ordinary
...
describe('fs.open / fs.promises.open', () => {
  it('fs.open routes through orivon.fs.open -- fs/tests/handle.test.ts covers the cursor/callback behaviour', async () => {
    const { openCalls } = installFakeOrivon()
    const fs = await import('../fs.js')
...
describe('mkdir / readdir / stat / rm / rename', () => {
  ...
  it('stat wraps the flat FileStat into Node-shaped Stats with real methods', async () => {
    ...
    const stat = await new Promise<import('../stats.js').NodeStats>((resolve, reject) => {
      fs.stat('piece-0', (err, result) => (err !== null ? reject(err) : resolve(result as import('../stats.js').NodeStats)))
    })
...
describe('fs\'s other members -- named refusal instead of absence (A135), reading one is safe (A169)', () => {
  ...
  // createReadStream/createWriteStream are real now (fs/streams.ts,
  // fs/tests/streams.test.ts) -- see this file's own default export for why
