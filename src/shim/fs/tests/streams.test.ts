// Exercises fs.createReadStream/createWriteStream (fs/streams.ts) --
// both real stream.Readable/Writable subclasses driven by fs/handle.ts's
...
// fs/tests/handle.test.ts. vitest.config.ts resolves that module's `stream` to
...
describe('fs.createReadStream', () => {
  it('opens read-only and streams the whole file as Buffer chunks by default', async () => {
    ...
    const { createReadStream } = await import('../streams.js')
...
describe('fs.createWriteStream', () => {
  it('opens with flags \'w\' by default and writes chunks positionally from 0', async () => {
    ...
    const { createWriteStream } = await import('../streams.js')
