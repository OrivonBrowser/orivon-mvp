// fs.promises -- built on fs/core.ts's SAME async core fs/fs.ts's
// callback family uses (code-guidelines.md Rule 3), plus fs.promises.open
// (fs/handle.ts, already built). `constants` mirrors real Node, which
...
import { encodingOf } from '../encoding.js'
