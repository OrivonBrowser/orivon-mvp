...
export const SHIM_MODULE_MAP: readonly ShimModuleEntry[] = [
  ...
  { specifier: 'net', status: 'ready', kind: 'local', implementation: './node-net.js', note: 'Queue item 3.2: net.connect/createConnection as a real stream.Duplex over orivon.net.connect, plus net.isIP (a confirmed k-rpc-socket caller, not a socket method -- see README.md requirement 1). net.createServer/net.Server (A114/d-0028, node-net-server.ts) are now real too, over orivon.net.listen -- a single-flight accept loop preserves the broker\'s one-read-one-accept credit rule. See src/shim/tests/node-net-socket.test.ts and node-net-server.test.ts.' },
  ...
  { specifier: 'dns', status: 'ready', kind: 'local', implementation: './node-dns.js', note: 'dns.lookup/dns.promises.lookup now resolve for real, over orivon.net.lookup (d-0030) -- the one confirmed caller, k-rpc-socket\'s `_resolveAndQuery`, only reaches this for a hostname bootstrap peer (net.isIP routes a literal-IP peer around it). A152: a denied resolution surfaces err.code === \'denied\', never a generic \'internal\'. See src/shim/tests/node-dns.test.ts.' },
  { specifier: 'fs', status: 'ready', kind: 'local', implementation: './node-fs.js', note: 'Queue item 3.2: readFile/writeFile/mkdir/readdir/stat/rm/rename as callback-style fs over the matching orivon.fs promises, plus the real readFileSync (ADR-0016\'s one synchronous exception). fs.open/fs.promises.open (node-fs-handle.ts) are now real too, over orivon.fs.open -- a local cursor reconstructs Node\'s implicit `position: null` on top of the contract\'s deliberately explicit-position-only FileHandle. FileHandle.createReadStream/createWriteStream refuse loudly, citing A184 (the broker\'s readable()/writable() are not page-reachable yet). See src/shim/tests/node-fs-handle.test.ts.' },
