// Exercises net/net.ts itself -- the module-map target a real `import 'net'`
// resolves to -- rather than net/socket.ts's Socket class
// (net/tests/socket.test.ts's job) or net/server.ts's Server class
// (net/tests/server.test.ts's job). Same stubbed-globalThis.orivon pattern as
// http/tests/modules.test.ts.
...
describe('net/net.ts', () => {
  ...
  it('names any other unbuilt member, reason unimplemented, when called -- reading it first does not throw', async () => {
    installFakeOrivon()
    const net = (await import('../net.js')).default as unknown as Record<string, () => unknown>
