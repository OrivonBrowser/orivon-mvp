...
// concern, not a branch inside fs-capability.ts, because it is authorised
...
// `confinePath` (./policy/paths.js) fs-capability.ts's `confineForOrigin`
...
// confinement implementation is the bug fs-capability.ts's own header says
...
/** Builds the `userSelected` member of `Broker['fs']` -- see this file's header for why it is its own capability rather than a branch inside fs-capability.ts. */
export function createUserSelectedCapability (
   ...
   * sufficient" reasoning `fs-capability.ts`'s own `open` doc gives for a
  ...
  function toFailableDirectoryHandle (key: string, entry: HandleEntry, root: string): FailableDirectoryHandle {
    ...
    return {
      ...
      // BOTH sides confined independently -- fs-capability.ts's own `rename`
      ...
      writeFile: async (path, data) => {
        ...
        // fs-capability.ts's own `writeFile` -- a picked folder's bytes
