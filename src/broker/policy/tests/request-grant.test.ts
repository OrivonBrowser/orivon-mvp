...
describe('decideGrantRequest -- a request may never declare a shape the manifest grammar itself would refuse', () => {
  ...
  // this version -- app.requestGrant must never mint one dynamically, even
  // when the manifest declares exactly the origin being asked for.
  it('refuses web.context even when the manifest declares exactly the requested origin', () => {
  ...
  it('refuses web.context with no requested patterns at all (the "whatever is declared" shorthand)', () => {
    const manifest = manifestWith({ web: { contexts: ['https://example.com'] } })
    const decision = decideGrantRequest(manifest, 'web.context', undefined)
    expect(decision).toEqual({ allowed: false, patterns: [] })
  })

  it('refuses web.context when the manifest never declares it either', () => {
    const manifest = manifestWith({})
    const decision = decideGrantRequest(manifest, 'web.context', ['https://example.com'])
    expect(decision).toEqual({ allowed: false, patterns: [] })
  })
