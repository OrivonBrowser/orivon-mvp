...
// the same Rule 2 seam capabilities/net.ts and capabilities/id.ts already
 ...
 * any of it. `userSelected` is the one member ./user-selected.ts
...
export function createFsCapability ({ deps, handleTable, ledger, canonical }: FsCapabilityOptions): Omit<Broker['fs'], 'userSelected'> {
  ...
  // ./user-selected.ts, which needs the identical wrapping for a
   ...
   * design notes on `capabilities/net.ts`): check `signal.aborted` before the
