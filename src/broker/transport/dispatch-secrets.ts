...
// ./dispatch-id.ts's header for the seam this and its siblings share.
...
/** The `secrets.*` slice of `ControlMethod` -- see ./dispatch-app.ts's own `AppControlMethod` for why this is derived rather than retyped. */
...
export async function dispatchSecrets (
  ...
  switch (method) {
    ...
    default: {
      // Exhaustiveness check, same reasoning and shape as ./dispatch-id.ts's own.
