...
// ./app.ts's header for the seam this and its siblings
// (transport/dispatch/id.ts, transport/dispatch/net.ts, transport/dispatch/fs.ts) share.
//
// NO PORT TRANSPORT, UNLIKE net.connect -- exactly transport/dispatch/id.ts's own
...
export async function dispatchWeb (
  ...
  switch (method) {
    ...
    default: {
      // Exhaustiveness check, same reasoning and shape as ./id.ts's
