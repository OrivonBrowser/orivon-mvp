...
// over its dedicated port -- ./socket.ts's and ./datagram.ts's
...
export function createServerRelay (options: ServerRelayOptions): ServerRelay {
  ...
  // Same tolerance, and the same reason, as ./socket.ts's failSocket:
  ...
  port.onMessage((raw) => {
    ...
    // handles, matching ./datagram.ts's stance toward a byte-path
  ...
  // triggers this without ever calling net.close -- see ./socket.ts's
  // own note; the same T11b reasoning applies to an abandoned server.
  port.onClose(() => { stop() })

  // UNCONDITIONAL, matching ./datagram.ts's reasoning, not
  // ./socket.ts's conditional one: see this file's own header.
  server.onUnlink((_reason, code) => { stop(code) })

  // The .catch is not decoration -- see ./socket.ts's identical note:
