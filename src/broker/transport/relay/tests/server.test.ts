...
import { createServerRelay } from '../server.js'
import { fakeMultiTransport, fakePort, fakeTcpServer, fakeTcpSocket, tick } from '../../tests/ipc.test-helpers.js'
...
describe('createServerRelay -- teardown', () => {
  ...
  it('is UNCONDITIONAL on unlink reason, unlike transport/relay/socket.ts -- a TcpServer has no write queue to truncate', async () => {
    const fake = fakeTcpServer()
    const port = fakePort()
    const transport = fakeMultiTransport()
    ...
    // transport/relay/socket.ts's own 'closed'/'sessionEnded' branch is LEFT ALONE --
  ...
  it('the renderer-side port closing frees the server\'s registry slot AND actually releases the underlying handle -- fails against the unfixed code, which only cleared the registry', async () => {
    ...
    // OS listening socket closing (transport/relay/server.ts's own fix comment). The
