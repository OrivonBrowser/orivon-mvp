...
function fakeAccepted (id: string): FailableTcpSocket {
  ...
  // accepted connection as an opaque item, exactly as ./server-relay.ts's own
  // onAccepted (which does the real work) expects.
  return { id } as FailableTcpSocket
}

// highWaterMark: 0 ON EVERY STREAM BELOW, matching net-capability.ts's own
...
/** A connections stream that enqueues `items`, then errors instead of closing -- mirrors net-capability.ts's own pull(), which maps a raw accept() failure to an OrivonError before erroring. */
...
describe('createAcceptPump -- terminal states', () => {
  ...
  it('stop() sends a terminal end message immediately and releases the reader, without waiting on demand', async () => {
    ...
    const pump = createAcceptPump({
      ...
      // shape socket-relay.test.ts uses for its own stop() test.
