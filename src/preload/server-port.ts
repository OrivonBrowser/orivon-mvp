...
/** One accepted connection, as this module hands it to orivon-surface.ts -- the socket-port.ts-shaped state machine plus the descriptor fields AcceptedMessage carried up front. */
...
export function createServerPort (options: ServerPortOptions): ServerPort {
  ...
  // Handled on THIS reference only, matching ./socket-port.ts's own `closed`
  ...
  port.onMessage((raw) => {
    ...
    switch (message.kind) {
      ...
      // port -- ./socket-port.ts and ./datagram-port.ts own those. Listed
