...
/**
 * What ./web.ts's webOpenContext bridge closure resolves to
 ...
 * ./web.ts's own header for the `web.awaitClose` long-poll it is
 * built from.
 */
...
/**
 * What surface/orivon.ts's fsUserSelectedDirectory bridge closure resolves
...
/**
 * What surface/orivon.ts's netUdpBind closure resolves to -- ../ports/datagram.ts's
...
/**
 * What surface/orivon.ts's netListen closure resolves to -- ../ports/server.ts's
...
/** The shape ../ports/socket.ts's SocketPort plus a connection descriptor and the three control-channel operations net.connect doesn't otherwise expose -- what surface/orivon.ts's netConnect bridge closure resolves to. Also what one accepted connection's own bridge is shaped as (surface/orivon.ts's buildAcceptedBridge) -- an accepted socket and a dialled one are the same thing once accepted. */
export interface MainWorldSocketBridge {
  ...
  /** Fires once if the write direction fails outright, or the port goes silent past the timeout -- see ../ports/socket.ts's own SocketPort.onFatal. */
  readonly onFatal: (cb: (code: OrivonErrorCode) => void) => void
  readonly closed: Promise<void>
  readonly close: () => Promise<void>
...
/**
 * The full shape `surface/orivon.ts` passes as installOrivon's `bridge`
