...
// SERIALISED into the main world (see ./routed-wire.ts's header) and
// publishes `core` on the shared slot; `releaseRoutedSlot` removes that slot
// once every installer has taken what it needs.
import type { ExtractedBody, FetchRouteTarget, ResponseHead, RoutedRequest, RoutedSlot, RoutedSocket } from './fetch-route-types.js'

/** Mirrors the literals `installRoutedCore` keeps inside its own body; see ./routed-wire.ts's ROUTED_MAX_HEAD_BYTES for why they exist twice. */
