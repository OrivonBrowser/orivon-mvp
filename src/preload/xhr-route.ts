...
// routed exchange as fetch() (./routed-core.ts), with the XHR spec's events,
...
//
// `installXhrRoute` is SERIALISED into the main world (see ./routed-wire.ts's
// header); the response views are ./xhr-route-response.ts's, the handler
// attributes ./routed-events.ts's.
import type { FetchRouteTarget, RoutedSlot, XhrBody } from './fetch-route-types.js'
