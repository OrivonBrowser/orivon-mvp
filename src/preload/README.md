...
| `settings.ts` | **only** the permissions panel's own view (`src/main/permissions/permissions-panel.ts`) | `orivonSettings`: list each app's grants and revoke one, and list each site's notification answer and reset one, after checking `location.href` against its expected URL; `src/main/ipc/settings-ipc.ts` re-verifies the sender on every call |
...
- **Workers and iframes get neither routing nor the shim's globals.** A preload runs only in a
  ...
  `orivon`, `process`, `setImmediate` or `Buffer`. An app that moves its network calls into a worker
  loses routing there.
