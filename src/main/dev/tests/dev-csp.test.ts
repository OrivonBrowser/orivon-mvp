...
const { devCspListener, installDevCsp, withAppendedCsp } = await import('../dev-csp.js')
...
async function run (listener: ReturnType<typeof devCspListener>, input: OnHeadersReceivedListenerDetails): Promise<HeadersReceivedResponse> {
...
describe('devCspListener', () => {
  it('gives a document from the dev origin the installed path\'s policy, built fresh per response', async () => {
    let csp = CSP
    const listener = devCspListener(ORIGIN, async () => csp)
  ...
  it('leaves every non-document response untouched', async () => {
    ...
    const listener = devCspListener(ORIGIN, cspFor)
  ...
  it('answers without a policy rather than hanging the response when the grant read fails', async () => {
    const listener = devCspListener(ORIGIN, async () => { throw new Error('broker gone') })
    expect(await run(listener, details({}))).toEqual({})
  })
})

describe('installDevCsp', () => {
  it('installs one listener on the dev origin\'s own app partition, building the installed path\'s policy from the live grants', async () => {
    const grants = [{ id: 'g1', origin: ORIGIN, grantedAt: 0, capability: 'https.connect', patterns: ['api.example.com:443'] }]
    installDevCsp({ app: { grants: async () => grants } } as never, ORIGIN)
    expect(fromPartition).toHaveBeenCalledWith(partitionFor(ORIGIN))
    expect(onHeadersReceived).toHaveBeenCalledTimes(1)

    const listener = onHeadersReceived.mock.calls[0]?.[0] as ReturnType<typeof devCspListener>
