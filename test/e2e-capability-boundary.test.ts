...
it('grants a real round trip and denies a real out-of-manifest connection', async () => {
  // scripts/smoke.mjs's own rule 1, carried over here: this test REPORTS,
  // it does not just trust its exit code. Every outcome below is recorded
  // into `checks`, printed as JSON before any assertion that could throw,
  // so a partial run still says exactly what happened.
  ...
  // ---- Phase 1: the real shell, the real page, the real (documented) gap
  ...
  // ---- Phase 2: the real broker, real I/O, granted vs. denied
  ...
  if (failures.length > 0) {
    console.error('\ne2e-capability-boundary FAILED:')
    for (const f of failures) console.error(`  - ${f}`)
  } else {
    console.log('\ne2e-capability-boundary passed.')
  }
